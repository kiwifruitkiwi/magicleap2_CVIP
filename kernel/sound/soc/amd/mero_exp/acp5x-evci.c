...
int acp_get_cmd_number(struct acp5x_dev_data *adata)
{
	int count = 0;
...
int acp_get_next_cmd_number(struct acp5x_dev_data *adata)
{
	int count = 0;
...
bool acp_remove_responses(struct acp5x_dev_data *adata,
			  bool remove_all,
...
int acp_add_response(unsigned int cmd_id, unsigned int cmd_number,
		     struct acp5x_dev_data *adata, unsigned int *p_event)
{
	unsigned int      index;
...
bool acp_record_response(struct acp5x_dev_data *adata,
		   ...
		   struct acp5x_dev_data *adata)
{
	...
	break;
	case ACP_EV_CMD_SET_I2S_MCLK_FREQ:
	{
		struct acpev_cmd_set_i2s_mclk_freq *cmd_buffer =
				(struct acpev_cmd_set_i2s_mclk_freq *)
				actual_command_buffer;
		cmd_buffer->cmd_number = acp_get_cmd_number(adata);
		cmd_num = cmd_buffer->cmd_number;
		acp_ev_command_id->cmd_size =
				sizeof(struct acpev_cmd_set_i2s_mclk_freq);
		size = acp_ev_command_id->cmd_size +
		       sizeof(struct acpev_command);
		write_buffer(cmd_ring_buffer_info, cmd_ring_buffer, size,
			     write_cmd_buffer);
...
int  ev_send_cmd(void *ev_cmd, u32 ev_cmd_length, struct acp5x_dev_data *adata,
			 ...
			 unsigned short *acp_ev_cmd_id)
{
	...
	case ACP_EV_CMD_SET_I2S_MCLK_FREQ:
	{
		struct acpev_cmd_set_i2s_mclk_freq *p_cmd_buf =
			(struct acpev_cmd_set_i2s_mclk_freq *)actual_cmd_buf;
		*cmd_num = p_cmd_buf->cmd_number;
	}
	break;
	default:
...
int acp_ev_send_cmd_and_wait_for_response(struct acp5x_dev_data *adata,
					  void *p_ev_cmd,
					  unsigned int cmd_length)
{
	int   ret = 0;
