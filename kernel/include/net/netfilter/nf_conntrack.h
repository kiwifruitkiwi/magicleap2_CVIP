...
struct nf_conn {
	...
	spinlock_t	lock;
	/* This ct is considered dead when timeout exceeds jiffies */
	u64 timeout;
...
#define nfct_time_stamp ((u64)(jiffies))

/* jiffies until ct expires, 0 if already expired */
static inline unsigned long nf_ct_expires(const struct nf_conn *ct)
{
	u64 timeout = ct->timeout - nfct_time_stamp;

	if (ct->timeout < nfct_time_stamp)
		return 0;
...
{
	return ct->timeout < nfct_time_stamp;
}
