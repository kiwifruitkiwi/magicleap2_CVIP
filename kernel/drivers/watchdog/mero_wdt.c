...
#include <linux/cvip/cvip_plt/cvip_plt_wdt.h>
...
static struct mero_watchdog_device *wd_bases[NUM_MERO_WATCHDOGS];
...
void
wdt_keepalive_in_panic(void)
{
	int i;

	for (i = 0; i < NUM_MERO_WATCHDOGS; ++i) {
		if (wd_bases[i] != NULL) {
			iowrite32(wd_bases[i]->wdt_count, wd_bases[i]->wdt_base + WDT_WTDAT);
			iowrite32(wd_bases[i]->wdt_count, wd_bases[i]->wdt_base + WDT_WTCNT);
		}
	}
}
EXPORT_SYMBOL_GPL(wdt_keepalive_in_panic);
...
wdt_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(mw->wdt_clock)) {
		WDT_DEBUG("mero_wdt: Error in retrieve clk, defering probe\n");
		ret = -EPROBE_DEFER;
		goto ERROR_IRQ;
	}
	...
	/*
	 * We keep an array of the mero watchdog structure, so we can access it from the
	 * panic handler without a device context.
	 */
	if (mw->wdd.id < NUM_MERO_WATCHDOGS) {
		wd_bases[mw->wdd.id] = mw;
	} else {
		pr_warn("WARNING: Found watchdog id %d, with a max set to %d!\n",
			mw->wdd.id, NUM_MERO_WATCHDOGS);
	}
