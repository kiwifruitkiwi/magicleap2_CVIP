...
ATOMIC_NOTIFIER_HEAD(cvip_pci_ready_notifier_list);
...
int cvip_pci_flush_dram_try(uint32_t tries)
{
	int count = 0;
...
EXPORT_SYMBOL(cvip_pci_flush_dram_try);
...
static int cvip_pci_probe(struct platform_device *pdev)
{
	...
	atomic_notifier_call_chain(&cvip_pci_ready_notifier_list, 0, NULL);
