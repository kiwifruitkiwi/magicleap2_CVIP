				     ...
				     u32 *associativity, u32 max_way_size)
{
	...
	*associativity = cache_size / way_size;
	...
	pr_debug("Syscache: override cache size: %d bytes (%dKB)\n",
		cache_size, cache_size >> 10);
	pr_debug("Syscache: override line size: %d bytes\n", line_size);
	pr_debug("Syscache: override way size: %d bytes (%dKB)\n",
		way_size, way_size >> 10);
	pr_debug("Syscache: override associativity: %d\n", *associativity);
			   ...
			   const struct device_node *np)
{
	...
	ret = of_property_read_u32_array(np, "arm,tag-latency", tag,
					 ARRAY_SIZE(tag));
	if (ret)
		pr_debug("Syscache: cannot find arm,tag-latency property value\n");
	else
	...
	ret = of_property_read_u32_array(np, "arm,data-latency", data,
					 ARRAY_SIZE(data));
	if (ret)
		pr_debug("Syscache: cannot find arm,data-latency property value\n");
	else
	...
	ret = of_property_read_u32_array(np, "arm,filter-ranges", filter,
					 ARRAY_SIZE(filter));
	if (ret)
		pr_debug("Syscache: cannot find arm,filter-ranges property value\n");
	else
		if (filter[1]) {
	...
	if (ret) {
		pr_debug("Syscache: cannot find arm,double-linefill property value\n");
	} else {
		if (val)
	...
	if (ret) {
		pr_debug("Syscache: cannot find arm,double-linefill-incr property value\n");
	} else {
		if (val)
	...
	if (ret) {
		pr_debug("Syscache: cannot find arm,double-linefill-wrap property value\n");
	} else {
		if (!val)
	...
	if (ret) {
		pr_debug("Syscache: cannot find arm,prefetch-drop property value\n");
	} else {
		if (val)
	...
	if (ret) {
		pr_debug("Syscache: cannot find arm,prefetch-offset property value\n");
	} else {
	...
	if (ret) {
		pr_debug("Syscache: cannot find prefetch-data property value\n");
	} else {
		if (val)
	...
	if (ret) {
		pr_debug("Syscache: cannot find prefetch-instr property value\n");
	} else {
		if (val)
...
static int __init syscache_probe(struct platform_device *pdev)
{
...
static int __exit syscache_remove(struct platform_device *pdev)
{
...
static struct platform_driver mero_syscache_drv __refconst = {
	.driver = {
		.owner	= THIS_MODULE,
		.name	= "mero-syscache",
	...
	.remove		= __exit_p(syscache_remove),
	.suspend	= syscache_suspend,
	.resume		= syscache_resume,
};
module_platform_driver(mero_syscache_drv);
