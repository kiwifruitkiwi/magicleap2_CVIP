...
#include "dsp_manager_streamids.h"
...
// DSP code loading from kernel
// is currently disabled.
// This will be enabled along with
// the CR that provides support for
// kernel based DSP code loading.
#define KERNEL_DSP_CODE_LOAD	(1)
...
#define POSTCODE_INIT_MAJOR			(0x01)
#define POSTCODE_INIT_MINOR			(0x00)
#define POSTCODE_PANIC_MAJOR		(0xFF)
#define POSTCODE_PANIC_MINOR		(0xFF)
#define POSTCODE_BOOTED_MAJOR		(0xAA)
#define POSTCODE_BOOTED_MINOR		(0xBB)
#define POSTCODE_STEP_MINOR_OFFSET	(16)
#define POSTCODE_STEP_MINOR_MASK	(0xFF)
#define POSTCODE_STEP_MAJOR_OFFSET	(24)
#define POSTCODE_STEP_MAJOR_MASK	(0xFF)
#define DSP_POSTCODE_REG_SIZE		(0x10)

// DSP TCM (internal) address
#define DSP_C5_TCM_INTERNAL_ADDR	(0xFFE00000U)
#define DSP_Q6_TCM_INTERNAL_ADDR	(0xFFE80000U)
// DSP Postcode (internal) address
#define DSP_C5_STATUS_INTERNAL_ADDR (0xFFEF8000U)
#define DSP_Q6_STATUS_INTERNAL_ADDR (0xFFEEC040U)

// DSP TCM sizes
#define DSP_Q6_TCM_SIZE_BYTES		(512*1024)
#define DSP_C5_TCM_SIZE_BYTES		(1024*1024)

// DSP TCM Inbound PIF access address
#define DSP_TCM_EXTERNAL_PADDR(_core_id) \
	((_core_id < 6) ? \
		(DSP_Q6_TCM_INTERNAL_ADDR + (((u64) 0x28U + _core_id) << 32U)) :\
		(DSP_C5_TCM_INTERNAL_ADDR + (((u64) 0x2EU + (_core_id - 0x6)) << 32U)))

// DSP Postcode/Status Inbound PIF access address
#define DSP_STATUS_EXTERNAL_PADDR(_core_id) \
	((_core_id < 6) ? \
		(DSP_Q6_STATUS_INTERNAL_ADDR + (((u64) 0x28U + _core_id) << 32U)) :\
		(DSP_C5_STATUS_INTERNAL_ADDR + (((u64) 0x2EU + (_core_id - 0x6)) << 32U)))

// DSP TCM sizes
#define DSP_TCM_SIZE(_core_id) \
	((_core_id < 6) ? (DSP_Q6_TCM_SIZE_BYTES) : (DSP_C5_TCM_SIZE_BYTES))

#define PERF_MONITOR_BASE(core) \
	((core < 6) ? \
		(0x2000000 + 0x50000 + (0x10000 * (core))) :\
		(0x2000000 + 0x30000 + (0x10000 * (core - 0x6))))
#define PERF_MONITOR_SIZE (0x10000)

#define PERF_MON_VALUE(base, n)	((base) + (0x1080 + (n) * 0x4))

#define PERF_ERR_OK		0
...
#define INITIALIZATION_TIMEOUT_US		(5000)
#define INITIALIZATION_POLL_DELAY_US	(50)

static const int initialization_max_attempts =
	INITIALIZATION_TIMEOUT_US / INITIALIZATION_POLL_DELAY_US;

#define ALL_ONES_MEM_PATTERN_32BIT  0xFFFFFFFF
#define ALL_ZEROS_MEM_PATTERN_32BIT       0x0

static BLOCKING_NOTIFIER_HEAD(dsp_helper_off_notifier_list);
...
static int  dsp_test_memory(int dsp_id, __iomem void *addr, uint32_t lenBytes);
...
static int	dsp_helper_get_status(struct dsp_state_t *states);
...
static const struct file_operations dsp_fops = {
	...
	.compat_ioctl = dsp_ioctl,
};

struct dsp_mempool_info {
	uint32_t name;
	uint32_t mem_type;
	// Can be used to specify caching
	// For e.g. ION_FLAG_CVIP_SLC_CACHED_0
	// for SLC caching
	uint32_t ion_flags;
	size_t size;
	// arm kernel virtual address
...
enum dsp_mempool_size {
	kCodeManagerSizeQ6 = 0x200000, //2MB
	kCodeManagerSizeC5 = 0x200000, //2MB
	kCodeSharedSizeQ6 = 0x200000,  //2MB
	kCodeSharedSizeC5 = 0x200000,  //2MB
};

enum dsp_mempool_base {
	kCodeManagerVirtualBaseQ6 = 0xc4000000,
	kCodeManagerVirtualBaseC5 = 0xc6000000,
	kCodeSharedVirtualBaseQ6 = 0xC4200000,
	kCodeSharedVirtualBaseC5 = 0xC6200000,
};

enum dsp_mempool_name {
	kCodeManagerQ6,
	kCodeManagerC5,
	kCodeSharedQ6,
	kCodeSharedC5,
...
static struct iommu_domain *sid_domain[TOTAL_NR_OF_STREAM_IDS] = { NULL };

static struct dsp_mempool_info dsp_pools[] = {
	{
		.name = kCodeManagerQ6,
		.mem_type = ION_HEAP_CVIP_DDR_PRIVATE_ID,
		.ion_flags = ION_FLAG_CVIP_SLC_CACHED_0,
		.size = kCodeManagerSizeQ6,
		.fixed_dsp_v_addr = kCodeManagerVirtualBaseQ6,
		.arm_k_addr = NULL,
		.mapped_size = 0,
	},
	{
		...
		.mem_type = ION_HEAP_CVIP_DDR_PRIVATE_ID,
		.ion_flags = ION_FLAG_CVIP_SLC_CACHED_0,
		.size = kCodeManagerSizeC5,
		.fixed_dsp_v_addr = kCodeManagerVirtualBaseC5,
		.arm_k_addr = NULL,
		.mapped_size = 0,
	},
	{
		...
		.mem_type = ION_HEAP_CVIP_DDR_PRIVATE_ID,
		.ion_flags = ION_FLAG_CVIP_SLC_CACHED_0,
		.size = kCodeSharedSizeQ6,
		.fixed_dsp_v_addr = kCodeSharedVirtualBaseQ6,
		.arm_k_addr = NULL,
		.mapped_size = 0,
	},
	{
		...
		.mem_type = ION_HEAP_CVIP_DDR_PRIVATE_ID,
		.ion_flags = ION_FLAG_CVIP_SLC_CACHED_0,
		.size = kCodeSharedSizeC5,
		.fixed_dsp_v_addr = kCodeSharedVirtualBaseC5,
		.arm_k_addr = NULL,
		.mapped_size = 0,
	},
};
...
static void dsp_helper_get_postcode(struct dsp_state_t *states, u8 *step_major, u8 *step_minor)
{
	u32 value = readl_relaxed(states->postcode_register);
	*step_major = ((value >> POSTCODE_STEP_MAJOR_OFFSET) & POSTCODE_STEP_MAJOR_MASK);
	*step_minor = ((value >> POSTCODE_STEP_MINOR_OFFSET) & POSTCODE_STEP_MINOR_MASK);
}

static void dsp_helper_set_postcode(struct dsp_state_t *states, u8 step_major, u8 step_minor)
{
	u32 value;

	value = step_major << POSTCODE_STEP_MAJOR_OFFSET;
	value |= step_minor << POSTCODE_STEP_MINOR_OFFSET;
	writel_relaxed(value, states->postcode_register);
}
...
static int dsp_helper_get_status(struct dsp_state_t *states)
{
	int dsp_status = kDspUnknown;
	u32 control_status;
	u32 status_set_mask = ADB400_MASTER_PD_ACK |
							ADB400_SLAVE_PD_ACK |
							ADB400_DTIUP_ACK |
							ADB400_DTIDWN_ACK;
	control_status = readl_relaxed(states->control_register);

	if (!(control_status & RESET_RELEASE))
		dsp_status = kDspReset;
	else {
		if (control_status & CLKGATE_EN) {
			dsp_status = kDspClkGated;
		} else {
			control_status = readl_relaxed(states->status_register);
			if ((control_status & status_set_mask) == status_set_mask) {
				if (control_status & DSP_ACTIVE_STATUS)
					dsp_status = kDspActiveBusy;
				else
					dsp_status = kDspActiveIdle;
			}
		}
	}

	DSP_DBG("return dsp_status=%d\n", dsp_status);
	return dsp_status;
}
...
int dsp_clk_dpm_cntrl(unsigned int clk_id, unsigned int dpm_level)
{
	...
	int ret = 0;
	bool is_q6_clk_set = false;
	bool is_c5_clk_set = false;
	...
	list_for_each(pos, &dsp_manager_dev->dsp_list) {
		...
		if (states->dsp_mode != DSP_OFF) {
			...
			if (((dsp_resources->dsp_id <= DSP_Q6_5) && !is_q6_clk_set) ||
				((dsp_resources->dsp_id > DSP_Q6_5) && !is_c5_clk_set)) {
				...
				if (dsp_resources->dsp_id <= DSP_Q6_5)
					is_q6_clk_set = true;
				else
					is_c5_clk_set =  true;
			}
				   ...
				   unsigned long clk_rate)
{
	...
	struct dsp_resources_t *dsp_resources = NULL;
	int ret = 0;
	unsigned long current_clk_rate = 0;
	...
	current_clk_rate = clk_get_rate(states->clk);
	if ((current_clk_rate * MHZ) == clk_rate)
		return ret;
	...
	if (states->clk_rate != DSP_CLK_OFF) {
		DSP_DBG("%s.%d: vote clk_rate:%lu\n", __func__, __LINE__, clk_rate);
		ret = clk_set_rate(states->clk, clk_rate);
	} else if (clk_rate == DSP_CLK_OFF) {
		DSP_DBG("%s.%d: vote clk_rate:0\n", __func__, __LINE__);
		ret = clk_set_rate(states->clk, DSP_CLK_OFF);
	}

	return ret;
}

int dsp_clk_cntrl(struct dsp_client_t *client, unsigned long arg)
{
	struct set_clk_t ioctl_param;
	bool is_q6_clk_set = false;
	bool is_c5_clk_set = false;
	bool q6_clk_check = false;
	bool c5_clk_check = false;
	unsigned long q6_clk_rate;
	unsigned long c5_clk_rate;
	struct dsp_state_t *states;
	struct list_head *pos;
	struct dsp_resources_t *dsp_resources = NULL;
	int ret = 0;
	...
	if (copy_from_user(&ioctl_param, (struct set_clk_t __user *)arg,
			   sizeof(ioctl_param))) {
		DSP_ERROR("Invalid IOCTL argument.\n");
		ret = -EFAULT;
		goto error;
	...
	q6_clk_rate = ioctl_param.q6_clk_rate_request_mhz * MHZ;
	c5_clk_rate = ioctl_param.c5_clk_rate_request_mhz * MHZ;
	...
	list_for_each(pos, &dsp_manager_dev->dsp_list) {
		...
		if (states->dsp_mode != DSP_OFF) {
			if (dsp_resources->dsp_id <= DSP_Q6_5 &&
				ioctl_param.q6_clk_change_request) {
				if (!is_q6_clk_set) {
					dsp_manager_dev->q6_cur_dpmclk
								= q6_clk_rate;
					ret = clk_set_rate(
						     states->clk, q6_clk_rate);
					if (ret) {
						DSP_ERROR("Failed to set q6 dsp clock rate.\n");
						goto error_res;
					}
					is_q6_clk_set = true;
				}
			} else if (ioctl_param.c5_clk_change_request) {
				if (!is_c5_clk_set) {
					dsp_manager_dev->c5_cur_dpmclk
								= c5_clk_rate;
					ret = clk_set_rate(
						     states->clk, c5_clk_rate);
					if (ret) {
						DSP_ERROR("Failed to set c5 dsp clock rate.\n");
						goto error_res;
					}
					is_c5_clk_set =  true;
				}
	...
	list_for_each(pos, &dsp_manager_dev->dsp_list) {
		...
		if (states->dsp_mode != DSP_OFF) {
			if (dsp_resources->dsp_id <= DSP_Q6_5) {
				if (!q6_clk_check) {
					ioctl_param.q6_clk_rate_current_mhz =
						     clk_get_rate(states->clk);
					q6_clk_check = true;
				}
				states->clk_rate =
				    ioctl_param.q6_clk_rate_current_mhz * MHZ;
			} else {
				if (!c5_clk_check) {
					ioctl_param.c5_clk_rate_current_mhz =
						     clk_get_rate(states->clk);
					c5_clk_check = true;
				}
				states->clk_rate =
				     ioctl_param.c5_clk_rate_current_mhz * MHZ;
			}
		}
	}

	if (copy_to_user((struct set_clk_t __user *)arg, &ioctl_param,
			 sizeof(ioctl_param))) {
...
int dsp_clk_gate(struct dsp_client_t *client, unsigned long arg)
{
	struct set_value_t ioctl_param;
	struct list_head *pos;
	struct dsp_state_t *states;
	struct dsp_resources_t *dsp_resources = NULL;
	int ret = 0;
	...
	if (copy_from_user(&ioctl_param, (struct set_value_t __user *)arg,
			   sizeof(ioctl_param))) {
		DSP_ERROR("Invalid IOCTL argument.\n");
		return -EFAULT;
	}
	...
	list_for_each(pos, &dsp_manager_dev->dsp_list) {
		dsp_resources = list_entry(pos, struct dsp_resources_t, list);
		if (ioctl_param.dsp_id == dsp_resources->dsp_id)
			break;
	...
	states = dsp_resources->states;
	dsp_status = dsp_helper_get_status(states);
	/*
	 * Only allow DSP to be clock gated when DSP is already active idle
	 * Only allow DSP clock to be ungated when DSP is already clock gated
	 */
	if ((ioctl_param.value && (dsp_status == kDspActiveIdle)) ||
		(!ioctl_param.value && (dsp_status == kDspClkGated))) {
		dsp_helper_write_clkgate_setting(states, ioctl_param.value);
	} else if ((ioctl_param.value && (dsp_status != kDspClkGated)) ||
		(!ioctl_param.value && !((dsp_status == kDspActiveIdle) ||
		(dsp_status == kDspActiveBusy)))) {
		// Only here if DSP in a different state than we are trying to write
		DSP_ERROR("Failed to write clock gate setting.\n");
...
static void dsp_helper_off_notify(int dsp_id)
{
	blocking_notifier_call_chain(&dsp_helper_off_notifier_list, dsp_id, NULL);
}
...
pd_end:
	...
	dsp_qreqn_pd_enable(device, states);
	states->dsp_mode = DSP_OFF;
	dsp_helper_off_notify(dsp_id);
	return ret;
}

void dsp_helper_off_register_notify(struct notifier_block *nb)
{
	blocking_notifier_chain_register(&dsp_helper_off_notifier_list, nb);
}
EXPORT_SYMBOL(dsp_helper_off_register_notify);

void dsp_helper_off_deregister_notify(struct notifier_block *nb)
{
	blocking_notifier_chain_unregister(&dsp_helper_off_notifier_list, nb);
}
EXPORT_SYMBOL(dsp_helper_off_deregister_notify);
		...
		int dsp_id)
{
	...
	u8 postcode_major;
	u8 postcode_minor;
	int attempts_remaining = initialization_max_attempts;
	...
	control_status = control_status | ENABLE_POWER | RUN_STALL;
	...
	dsp_helper_set_postcode(states, POSTCODE_INIT_MAJOR,
		POSTCODE_INIT_MINOR);

	/* Release run_stall */
	control_status &= ~RUN_STALL;
	writel_relaxed(control_status, states->control_register);
	...
	do {
		if (dsp_helper_read(states) == DSP_RUN) {
			...
			/*
			 * Wait for DSP to initialize.
			 * Watch the postcode register for successful init
			 */
			for (; attempts_remaining > 0; --attempts_remaining) {
				dsp_helper_get_postcode(states, &postcode_major,
					&postcode_minor);
				if ((postcode_major == POSTCODE_BOOTED_MAJOR &&
					postcode_minor == POSTCODE_BOOTED_MINOR) ||
					(postcode_major == POSTCODE_PANIC_MAJOR &&
					postcode_minor == POSTCODE_PANIC_MINOR))
					break;
				udelay(INITIALIZATION_POLL_DELAY_US);
			}

			if (!(postcode_major == POSTCODE_BOOTED_MAJOR &&
				postcode_minor == POSTCODE_BOOTED_MINOR)) {
				DSP_ERROR(" *** DSP%d failed boot: PC = 0x%02X.0x%02X ***\n",
					dsp_id, postcode_major,
					postcode_minor);
				break;
			}
			/* Successful boot */
			DSP_DBG(" *** DSP%d booted successfully ***\n", dsp_id);
			return 0;
		}
		udelay(INITIALIZATION_POLL_DELAY_US);
...
int dsp_helper_run_memory_test(struct dsp_device_t *device,
	struct dsp_state_t *states,	struct test_memory_t *test_params)
{
	int dsp_id;
	u32 control_status;
	int ret;
	...
	dsp_id = test_params->dsp_id;

	if (test_params->num_iterations <= 0) {
		DSP_ERROR("No test iterations specified for dsp%d.\n", dsp_id);
		return -EINVAL;
	}
	...
	test_params->pass = 1;
	for (i = 0; i < test_params->num_iterations; i++) {
		/* Do a memory test */
		tcm_err_cnt = dsp_test_memory(dsp_id,
			device->tcm[dsp_id], DSP_TCM_SIZE(dsp_id));
		if (tcm_err_cnt) {
			DSP_ERROR("FAIL: Iteration %2u had %d failures on DSP%d\n",
				i, tcm_err_cnt, dsp_id);
			test_params->pass = 0;
		} else {
			DSP_INFO("PASS: Iteration %2u had zero failures on DSP%d\n",
				i, dsp_id);
		}
	}
...
static int dsp_test_memory(int dsp_id, __iomem void *addr, uint32_t lenBytes)
{
	/* Perform march_c_32bit */
	uint32_t j;
	uint32_t *pMem = NULL;
	uint32_t length = lenBytes / sizeof(uint32_t);
	int err_count = 0;

	pMem = addr;
	/* Write 0s pattern(ascending) */
	for (j = 0; j < length; j++)
		pMem[j] = ALL_ZEROS_MEM_PATTERN_32BIT;

	/* Read 0s->Write 1s pattern(ascending) */
	for (j = 0; j < length; j++) {
		if (pMem[j] != ALL_ZEROS_MEM_PATTERN_32BIT) {
			DSP_ERROR("DSP%d MEM_ERR: @ %p, exp=0x%08X, act=0x%08X\n",
				dsp_id,
				&pMem[j], ALL_ZEROS_MEM_PATTERN_32BIT, pMem[j]);
			++err_count;
		}

		pMem[j] = ALL_ONES_MEM_PATTERN_32BIT;
	}

	/* Read 1s->Write 0s pattern(ascending) */
	for (j = 0; j < length; j++) {
		if (pMem[j] != ALL_ONES_MEM_PATTERN_32BIT) {
			DSP_ERROR("DSP%d MEM_ERR: @ %p, exp=0x%08X, act=0x%08X\n",
				dsp_id,
				&pMem[j], ALL_ONES_MEM_PATTERN_32BIT, pMem[j]);
			++err_count;
		}
	...
	/* Read 0s -> Write 1s pattern (descending) */
	...
	/* Read 1s->Write 0s pattern(descending) */
	for (j = (length - 1); j <= 0; j--) {
		if (pMem[j] != ALL_ONES_MEM_PATTERN_32BIT) {
			DSP_ERROR("DSP%d MEM_ERR: @ %p, exp=0x%08X, act=0x%08X\n",
				dsp_id,
...
static void dsp_memory_entry_deallocate_nolock(struct dsp_memory_t *memory)
{
	...
	if (memory->domain0 && memory->size_0) {
		CVIP_EVTLOG(dsp_manager_dev->event_log,
			"iommu_unmap domain0", memory->addr, memory->size);
	...
	for (i = 0; i < memory->mapcnt; i++) {
		if (memory->domain[i]) {
			CVIP_EVTLOG(dsp_manager_dev->event_log,
				"iommu_unmap domain", i, memory->addr, memory->size);
...
static void dsp_device_deallocate_nolock(struct dsp_device_t *dsp_device)
{
	...
	if (dsp_device) {
		...
		for (i = DSP_Q6_0; i < DSP_MAX; i++) {
			if (dsp_device->postcode_register[i])
				iounmap(dsp_device->postcode_register[i]);

			if (dsp_device->perf_register[i])
				iounmap(dsp_device->perf_register[i]);

			if (dsp_device->tcm[i])
				iounmap(dsp_device->tcm[i]);
		}
...
static struct dsp_device_t *dsp_device_allocate(void)
{
	...
	for (i = DSP_Q6_0; i < DSP_MAX; i++) {
		dsp_device->postcode_register[i] =
			ioremap(DSP_STATUS_EXTERNAL_PADDR(i),
				DSP_POSTCODE_REG_SIZE);
		if (!dsp_device->postcode_register[i]) {
			DSP_ERROR("Failed to map dsp postcode registers.\n");
			goto error;
		}

		dsp_device->tcm[i] =
			ioremap(DSP_TCM_EXTERNAL_PADDR(i),
				DSP_TCM_SIZE(i));
		if (!dsp_device->tcm[i]) {
			DSP_ERROR("Failed to map dsp tcm.\n");
			goto error;
		}

		dsp_device->perf_register[i] = ioremap(PERF_MONITOR_BASE(i), PERF_MONITOR_SIZE);
		if (!dsp_device->perf_register[i]) {
			DSP_ERROR("Failed to map dsp perf registers.\n");
			goto error;
		}
	}
	...
	for (i = DSP_Q6_0; i < DSP_MAX; i++) {
		...
		tmp_resource->states->postcode_register =
			dsp_device->postcode_register[i];

		tmp_resource->states->perf_register =
			dsp_device->perf_register[i];
	...
	/*
	 * Note: DPM3 for Q6 and DPM2 for C5 only work on
	 * Mero Secure parts fused with new DPM settings.
	 * Setting DSP frequencies to DPM2 clks.
	 */
	dsp_device->q6_cur_dpmclk = q6clk_dpm[2];
	dsp_device->c5_cur_dpmclk = c5clk_dpm[2];
...
static int ionbuf_arm_cache_invalidate(struct dma_buf *dmabuf)
{
	int i;
...
static int ionbuf_cvip_slc_invalidate(uint64_t start_address, uint32_t size_bytes)
{
	if (!size_bytes || !start_address) {
		DSP_ERROR("%s: invalid param\n", __func__);
		return -EINVAL;
	}

	return syscache_inv_range(start_address, start_address + size_bytes);
}

static int ionbuf_cvip_slc_flush(uint64_t start_address, uint32_t size_bytes)
{
	if (!size_bytes || !start_address) {
		DSP_ERROR("%s: invalid param\n", __func__);
		return -EINVAL;
	...
	return syscache_flush_range(start_address, start_address + size_bytes);
}


static int init_ion_buf_sg(int idx)
{
	int ret;
	...
	// device in order to get us the sgtable for smmu mapping.
	snprintf(biommu_sid_str, MAX_BIOMMU_STR_LEN_BYTES, "biommu!vdsp@%x", 0);

	iommudev_ret = biommu_get_domain_dev_byname(biommu_sid_str);
	if (IS_ERR_OR_NULL(iommudev_ret)) {
		DSP_ERROR("Failed to get device sid 0\n");
		ret = PTR_ERR(iommudev_ret);
		return ret;
	}

	dsp_pools[idx].dma_attach = dma_buf_attach(dsp_pools[idx].dmabuf,
						   iommudev_ret);
	if (IS_ERR_OR_NULL(dsp_pools[idx].dma_attach)) {
		DSP_ERROR("Failed to attach dev dmabuf\n");
		ret = PTR_ERR(dsp_pools[idx].dma_attach);
		return ret;
	}
	...
	dsp_pools[idx].sgtable = dma_buf_map_attachment(dsp_pools[idx].dma_attach,
							DMA_BIDIRECTIONAL);
	if (IS_ERR_OR_NULL(dsp_pools[idx].sgtable)) {
		DSP_ERROR("Failed to get sg dmabuf, idx=%d\n", idx);
		dma_buf_detach(dsp_pools[idx].dmabuf, dsp_pools[idx].dma_attach);
		ret = PTR_ERR(dsp_pools[idx].sgtable);
		return ret;
	}
...
// Must be called while holding a mutex in process
// context.
static int map_dsp_code_ionbuf_to_sids(void)
{
	int i;
	int j;
	int ret;
	const int num_pools = sizeof(dsp_pools) / sizeof(struct dsp_mempool_info);
	...
	// If the first is not initialized, it means none
	// of the sid_domains are initialized.
	if (!sid_domain[0]) {
		for (j = 0; j < TOTAL_NR_OF_STREAM_IDS; ++j) {
			sid = cvcore_stream_ids[j];
			snprintf(biommu_sid_str, MAX_BIOMMU_STR_LEN_BYTES,
				"biommu!vdsp@%x", sid);

			sid_domain[j] = biommu_get_domain_byname(biommu_sid_str);
			if (IS_ERR_OR_NULL(sid_domain)) {
				DSP_ERROR("Failed to get device for sid=0x%x\n", sid);
	...
	for (i = 0; i < num_pools; ++i) {
		for (j = 0; j < TOTAL_NR_OF_STREAM_IDS; ++j) {
			...
			// Set read-only access for dsp code regions
			// for production builds
			iommu_prot = IOMMU_READ;
			#else
			// DSPs will have read and write access to
			// dsp code region only non-production builds.
			iommu_prot = IOMMU_READ | IOMMU_WRITE;
			#endif

			mapped_size = iommu_map_sg(sid_domain[j],
						   dsp_pools[i].fixed_dsp_v_addr,
						   dsp_pools[i].sgtable->sgl,
						   dsp_pools[i].sgtable->nents,
						   iommu_prot);

			if (mapped_size <= 0) {
				DSP_ERROR("Failed to create iommu map for sid=0x%x\n",
					cvcore_stream_ids[j]);
				return -EFAULT;
			}
		}

		dsp_pools[i].mapped_size = mapped_size;
	}

	return 0;
}

static int init_dsp_code_regions(void)
{
	int i;
	int retval;
	const int num_pools = sizeof(dsp_pools) / sizeof(struct dsp_mempool_info);
	...
	for (i = 0; i < num_pools; ++i) {
		// alloc ion buffer
		dsp_pools[i].fd = ion_alloc(dsp_pools[i].size,
			1 << dsp_pools[i].mem_type, dsp_pools[i].ion_flags);
		if (dsp_pools[i].fd < 0) {
			DSP_ERROR("Failed to allocated ION private ddr memory,i=%d\n", i);
			return -ENOMEM;
		}

		dsp_pools[i].dmabuf = dma_buf_get(dsp_pools[i].fd);
		if (IS_ERR_OR_NULL(dsp_pools[i].dmabuf)) {
			DSP_ERROR("Invalid fd unable to get dma buffer.\n");
			ksys_close(dsp_pools[i].fd);
			return -EINVAL;
		}

		// Invalidate before use
		retval = ionbuf_arm_cache_invalidate(dsp_pools[i].dmabuf);
		if (retval) {
			DSP_ERROR("arm cache inval failed for private ddr, ret=%d\n",
				retval);
			dma_buf_put(dsp_pools[i].dmabuf);
			ksys_close(dsp_pools[i].fd);
			return retval;
		}

		dsp_pools[i].arm_k_addr = dma_buf_kmap(dsp_pools[i].dmabuf, 0);
		if (!dsp_pools[i].arm_k_addr) {
			DSP_ERROR("Failed kmap for private ddr dmabuf\n");
			dma_buf_put(dsp_pools[i].dmabuf);
			ksys_close(dsp_pools[i].fd);
			return PTR_ERR(dsp_pools[i].arm_k_addr);
		}

		retval = init_ion_buf_sg(i);
		if (retval) {
			DSP_ERROR("init_ion_buf_sg failed with ret=%d\n", retval);
			return retval;
		}
	}
...
static void deinit_dsp_code_regions(void)
{
	int i;
	int j;
	const int num_pools = sizeof(dsp_pools) / sizeof(struct dsp_mempool_info);
	...
	for (i = 0; i < num_pools; ++i) {
		dma_buf_unmap_attachment(dsp_pools[i].dma_attach,
					 dsp_pools[i].sgtable,
					 DMA_BIDIRECTIONAL);
		dma_buf_detach(dsp_pools[i].dmabuf, dsp_pools[i].dma_attach);

		for (j = 0; j < TOTAL_NR_OF_STREAM_IDS; j++)
			iommu_unmap(sid_domain[j], dsp_pools[i].fixed_dsp_v_addr,
				    dsp_pools[i].mapped_size);
		dsp_pools[i].mapped_size = 0;
	}
}

int read_write_dsp_code_regions(struct dsp_client_t *client, unsigned long arg)
{
	struct dsp_binary_info_t ioctl_param;
	int i;
	void *dsp_code_buf_ptr = NULL;
	uint64_t pool_range_start;
	uint64_t pool_range_end;
	uint64_t load_start;
	uint64_t load_end;
	const int num_pools = sizeof(dsp_pools) / sizeof(struct dsp_mempool_info);
	...
	if (copy_from_user(&ioctl_param, (struct dsp_binary_info_t __user *)arg,
			   sizeof(ioctl_param))) {
		DSP_ERROR("Invalid IOCTL argument.\n");
		return -EFAULT;
	}
	...
	load_start = ioctl_param.vaddress;
	load_end = (uintptr_t)ioctl_param.vaddress + ioctl_param.size;

	for (i = 0; i < num_pools; ++i) {
		if (dsp_pools[i].mapped_size <= 0)
			return -EFAULT;

		pool_range_start = (uintptr_t)dsp_pools[i].fixed_dsp_v_addr;
		pool_range_end = pool_range_start + dsp_pools[i].size - 1;

		if ((load_start >= pool_range_start) && (load_end <= pool_range_end)) {
			dsp_code_buf_ptr =
				(void *)(dsp_pools[i].arm_k_addr + (load_start - pool_range_start));

			if (ionbuf_arm_cache_invalidate(dsp_pools[i].dmabuf)) {
				DSP_ERROR("arm cache inval failed for private ddr\n");
				return -EFAULT;
			}

			if (ionbuf_cvip_slc_invalidate(ioctl_param.vaddress, ioctl_param.size)) {
				DSP_ERROR("SLC inval failed for private ddr\n");
				return -EFAULT;
			}

			if (ioctl_param.access_type == MemoryAccessWrite) {
				if (copy_from_user(dsp_code_buf_ptr, ioctl_param.ptr_binary,
						ioctl_param.size)) {
					DSP_ERROR("Invalid IOCTL argument.\n");
					return -EFAULT;
				}
			} else if (ioctl_param.access_type == MemoryAccessRead) {
				if (copy_to_user((struct dsp_binary_info_t __user *) arg,
						&ioctl_param, sizeof(ioctl_param))) {
					DSP_ERROR("Unable to copy to user.\n");
					return -EFAULT;
				}

				if (copy_to_user(ioctl_param.ptr_binary, dsp_code_buf_ptr,
						ioctl_param.size)) {
					DSP_ERROR("Unable to copy to user.\n");
					return -EFAULT;
				}
			...
			//SLC flushing should not be required,
			//useful for debug view of physical backing memory.
			if (ionbuf_cvip_slc_flush(ioctl_param.vaddress,
						       ioctl_param.size)) {
				DSP_ERROR("Failed In DSP SLC Cache Flush in RW IOCTL\n");
				return -EFAULT;
			}
		}
	}
...
static int __init dsp_init(void)
{
	...
	if (KERNEL_DSP_CODE_LOAD) {
		// Create and smmu map DSP_CODE regions
		ret = init_dsp_code_regions();
		if (ret) {
			DSP_ERROR("init_dsp_code_regions failed, ret=%d\n", ret);
...
static int dsp_open(struct inode *inode, struct file *file)
{
	...
	if (KERNEL_DSP_CODE_LOAD) {
		// Checking just first pool is sufficient enough to know
		// if we have already called map_dsp_code_ionbuf_to_sids
		// or not.
		if (!dsp_pools[0].mapped_size) {
			ret = map_dsp_code_ionbuf_to_sids();
			if (ret) {
				DSP_ERROR("Failed to map dsp code for sids\n");
				goto post_client_error;
			}
		}
	}
...
static long dsp_mem_map(struct dsp_client_t *client, unsigned long arg, bool allocated)
{
	...
	if (internal) {
		...
		while (access_attr && (mapcnt < DSPMEM_MAP_NUM)) {
			...
			iommu_prot = IOMMU_READ;
			#else
			// Enable read-write access for non-production builds.
			iommu_prot = IOMMU_READ | IOMMU_WRITE;
			#endif
...
static long dsp_run_test(struct dsp_client_t *client, unsigned long arg)
{
	struct test_memory_t ioctl_param;
	struct list_head *pos;
	struct dsp_resources_t *dsp_resources = NULL;
	int ret;
	int dspid = -1;
	...
	if (copy_from_user(&ioctl_param, (struct test_memory_t __user *)arg,
		sizeof(ioctl_param))) {
		DSP_ERROR("Invalid IOCTL argument.\n");
		ret = -EFAULT;
		goto error;
	...
	CVIP_EVTLOG(dsp_manager_dev->event_log, "->DSP_RUN_TEST", dspid,
		ioctl_param.test_opt, 0x1111);
	ret = mutex_lock_interruptible(&dsp_manager_dev->dsp_device_lock);
	DSP_DBG("IOCTL_DSP_RUN_TEST for dsp%d started.\n", dspid);
	if (ret) {
		DSP_ERROR("Failed to lock dsp device.\n");
		goto error;
	}
	...
	switch (ioctl_param.test_opt) {
	case DSP_TCM_MEMORY_MARCH_C_32:
		dsp_resources->states->submit_mode = DSP_RUN;
		ret = dsp_helper_run_memory_test(dsp_manager_dev,
			dsp_resources->states,
			&ioctl_param);

		dsp_resources->states->submit_mode = DSP_OFF;
		dsp_helper_off(dsp_manager_dev, dsp_resources);
		break;
	default:
		DSP_ERROR("Invalid DSP test type.\n");
		ret = -EINVAL;
	}
	...
	mutex_unlock(&dsp_resources->dsp_resource_lock);
	DSP_DBG("IOCTL_DSP_RUN_TEST for dsp%d successful.\n", dspid);
	CVIP_EVTLOG(dsp_manager_dev->event_log, "<-DSP_RUN_TEST", dspid,
		0x9999);

	if (copy_to_user((struct test_memory_t __user *)arg, &ioctl_param,
		sizeof(ioctl_param))) {
		DSP_ERROR("Invalid IOCTL argument.\n");
		ret = -EFAULT;
		goto post_lock_error;
...
post_lock_error:
	mutex_unlock(&dsp_resources->dsp_resource_lock);
error:
	CVIP_EVTLOG(dsp_manager_dev->event_log, "<-DSP_RUN_TEST",
		ioctl_param.dsp_id, 0xdead, -ret);
	return ret;
}
...
static long dsp_get_status(struct dsp_client_t *client, unsigned long arg)
{
	struct get_value_t ioctl_param;
	struct list_head *pos;
	struct dsp_resources_t *dsp_resources = NULL;
	int ret;
	...
	if (copy_from_user(&ioctl_param, (struct get_value_t __user *)arg,
			   sizeof(ioctl_param))) {
		DSP_ERROR("Invalid IOCTL argument.\n");
		ret = -EFAULT;
		return ret;
	...
	ret = mutex_lock_interruptible(&dsp_manager_dev->dsp_device_lock);
	DSP_DBG("IOCTL_DSP_GET_STATUS started.\n");
	if (ret) {
		DSP_ERROR("Failed to lock dsp device.\n");
		return ret;
	}
	...
	ioctl_param.value = dsp_helper_get_status(dsp_resources->states);
	mutex_unlock(&dsp_resources->dsp_resource_lock);
	if (copy_to_user((struct get_value_t __user *)arg, &ioctl_param,
			 sizeof(ioctl_param))) {
		DSP_ERROR("Unable to copy to user.\n");
		ret = -EFAULT;
	}
...
static u32 get_perf_cntr_val(void __iomem *base, int cntr_id)
{
	u32 val;

	val = readl_relaxed(PERF_MON_VALUE(base, cntr_id));
	DSP_DBG("CNTR%d:0x%x\n", cntr_id, val);

	return val;
}

static int dsp_get_perf_cntr(struct dsp_client_t *client, unsigned long arg)
{
	struct get_perf_cntr_t ioctl_param;
	struct list_head *pos;
	struct dsp_resources_t *dsp_resources = NULL;
	void __iomem *base;
	int ret;
	...
	if (copy_from_user(&ioctl_param, (struct get_perf_cntr_t __user *)arg,
			   sizeof(ioctl_param))) {
		DSP_ERROR("Invalid IOCTL argument.\n");
		ret = -EFAULT;
		goto error;
	...
	base = dsp_resources->states->perf_register;
	ioctl_param.val = get_perf_cntr_val(base, ioctl_param.cntr_id);

	if (copy_to_user((struct get_perf_cntr_t __user *)arg, &ioctl_param,
			 sizeof(ioctl_param))) {
		DSP_ERROR("Unable to copy to user.\n");
		ret = -EFAULT;
		goto post_lock_error;
	...
	mutex_unlock(&dsp_resources->dsp_resource_lock);
	DSP_DBG("successful. dsp-cntr-val[%d-%d-%d]\n",
		ioctl_param.dsp_id, ioctl_param.cntr_id,
		ioctl_param.val);
	return 0;
		      ...
		      unsigned long arg)
{
	...
	case IOCTL_DSP_GET_STATUS:
		 ...
		 * Return: DSP_STATUS
		 */
		CVIP_EVTLOG(dsp_manager_dev->event_log, "IOCTL_DSP_GET_STATUS");
		return dsp_get_status(client, arg);
	...
	case IOCTL_DSP_SET_CLK:
		/* _IOW
		 * Params: Q6_CLK_RATE, C5_CLK_RATE
		 * Return: Pass or Fail
		 */
		CVIP_EVTLOG(dsp_manager_dev->event_log, "IOCTL_DSP_SET_CLK");
		return dsp_clk_cntrl(client, arg);
	case IOCTL_DSP_CLK_GATE:
		/* _IOW
		 * Params: CLK_GATE_SETTING
		 * Return: Pass or Fail
		 */
		CVIP_EVTLOG(dsp_manager_dev->event_log, "IOCTL_DSP_CLK_GATE");
		return dsp_clk_gate(client, arg);
	case IOCTL_DSP_GET_PERF_CNTR:
		/* _IOWR
		 * Params: DSP_ID, CNTR_ID
		 * Return: Counter value
		 */
		CVIP_EVTLOG(dsp_manager_dev->event_log, "IOCTL_DSP_GET_PERF_CNTR");
		return dsp_get_perf_cntr(client, arg);
	case IOCTL_DSP_RUN_TEST:
		/* _IOWR
		 * Params: DSP_ID, TEST_OPT, NUM_ITERATIONS
		 * Return: Pass or Fail
		 */
		CVIP_EVTLOG(dsp_manager_dev->event_log, "IOCTL_DSP_RUN_TEST");
		return dsp_run_test(client, arg);
	case IOCTL_DSP_BINARY_RW:
		/* _IOWR
		 * Params: DSP_BINARY_VADDR, DSP_BINARY_PTR, DSP_BINARY_SIZE
		 * Return: Pass or Fail
		 */
		CVIP_EVTLOG(dsp_manager_dev->event_log, "IOCTL_DSP_LOAD_BINARY");
		return read_write_dsp_code_regions(client, arg);
	default:
