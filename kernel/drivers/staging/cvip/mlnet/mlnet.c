...
#ifdef MLNET_ARM
    #define MLNET_PROC_IS_ARM (true)
#else
    #define MLNET_PROC_IS_ARM (false)
#endif
...
static const char mac_address_x86[ETH_ALEN] = { 0x02, 0x86, 0x86, 0x86, 0x86, 0x86 };
static const char mac_address_arm[ETH_ALEN] = { 0x02, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa };
...
static ssize_t set_nbd_server_ready(struct device *dev,
			    struct device_attribute *attr, const char *buf, size_t count)
{
	uint32_t msg = MLNET_NBD_SERVER_READY;
...
static ssize_t set_nfs_server_ready(struct device *dev,
			    struct device_attribute *attr, const char *buf, size_t count)
{
	uint32_t msg = MLNET_NFS_SERVER_READY;
...
static ssize_t set_mlnet_setup_complete(struct device *dev,
			    struct device_attribute *attr, const char *buf, size_t count)
{
	uint32_t msg = MLNET_MLNET_SETUP_COMPLETE;
...
static int mlnet_net_init(void)
{
	...
	if (MLNET_PROC_IS_ARM)
		memcpy(mdev.netdev->dev_addr, mac_address_arm, ETH_ALEN);
	else
		memcpy(mdev.netdev->dev_addr, mac_address_x86, ETH_ALEN);
