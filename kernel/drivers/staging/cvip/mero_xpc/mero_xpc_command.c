...
static void xpc_command_save_wait_point(XpcAsyncTicket ticket)
{
	...
	mb_ptr =
		xpc_get_global_mailbox_from_global_mailbox_idx(ticket);
	/*
	 * Never expect mb_ptr to be NULL. Not much we can do
	 * to handle it, so let's BUG.
	 */
			      ...
			      enum xpc_client_api_mode client_mode)
{
	...
	pl320_status = pl320_acquire_mailbox(&mailbox_info.pl320_info,
	...
	pl320_status = pl320_send(&(mailboxes[mb_idx].mailbox_info.pl320_info),
		route_solution.first_route.ipc_target_irq_mask, message->data,
		message->length);
					  ...
					  int timeout_us)
{
	...
	if (status == -ERESTARTSYS) {
		xpc_debug(1, "xpc_command_wait_response_async_timed - restart_sys\n");
		return -ERESTARTSYS;
	}
			   ...
			   XpcAsyncTicket *ticket)
{
	...
	status = xpc_send_command_internal(&request, true, XPC_CLIENT_IS_INDIRECT);
	if (status == 0) {
				 ...
				 uint8_t *command, size_t command_length)
{
	...
	return xpc_send_command_internal(&request, false, XPC_CLIENT_IS_INDIRECT);
}
		     ...
		     XpcHandlerArgs args)
{
	...
	xpc_set_global_mailbox_pid(claimed_mailbox, current->pid,
				   XPC_REQUEST_IS_TARGET, XPC_CLIENT_IS_INDIRECT);
