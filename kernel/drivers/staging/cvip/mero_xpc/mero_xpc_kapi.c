...
static void xpc_drv_release_by_pid(pid_t pid)
{
	...
	for (ipc_id = 0; ipc_id < XPC_NUM_IPC_DEVICES; ipc_id++) {
		...
		for (mb_idx = 0; mb_idx < XPC_IPC_NUM_MAILBOXES;
			mb_idx++) {
			global_mb_idx =
				ipc_id * XPC_IPC_NUM_MAILBOXES + mb_idx;
			/**
					   ...
					   struct xpc_client_t *client)
{
	...
	if (client->use_kernel_api) {
		...
		retval = xpc_send_command_internal(&req, true, XPC_CLIENT_IS_DIRECT);
	}
	...
	struct xpc_client_t* client)
{
	...
	if (client->use_kernel_api) {
		retval = xpc_command_send_async(req.command.channel,
			xpc_channel_mask_to_idx(req.command.dst_mask),
			req.command.data, req.command.length,
			&req.command.ticket);
	}
	else {
		retval = xpc_send_command_internal(&req.command, true, XPC_CLIENT_IS_DIRECT);
	}
	...
	if (client->use_kernel_api) {
		retval =
			xpc_command_wait_response_async(&req.response.ticket, req.response.data,
				XPC_MAX_RESPONSE_LENGTH,
				&req.response.length);
	}
	else {
	...
	case XPC_MODE_COMMAND:
		if (irq_cause == PL320_IRQ_REASON_DESTINATION_INT) {
			...
			if (command_handlers[header.fields.vchannel] == NULL) {
				...
				queue_is_empty = list_empty_careful(
