...
static int xdebug_configure(uint8_t ipc_dev_mask, int threshold, int period_us, int ratelimit_event_count, int ratelimit_timeout_us)
{
    ...
    atomic_set(&(xdebug_struct.ratelimit_event_count), ratelimit_event_count);
    atomic_set(&(xdebug_struct.ratelimit_timeout_us), ratelimit_timeout_us);

    xpc_debug(0, "Configured xdebug: IPC=0x%02X, Threshold=%d, Sample=%dus,"
        " Rate-limit=%d, Rate-limit Timeout=%dus\n", ipc_dev_mask, threshold,
        period_us, ratelimit_event_count, ratelimit_timeout_us);
    return 0;
}
...
int xdebug_parse_command(const char *buf)
{
    ...
    ret = sscanf(buf, "%c:%c:%i:%i:%i:%i:%i", &cmd, &sub_cmd, &opt1, &opt2, &opt3, &opt4, &opt5);
    ...
    if (sub_cmd == 'e') {
        ...
        if (ret != 2 && ret != 3 && ret != 4 && ret != 5 && ret != 6 && ret != 7) {
            return -EINVAL;
        }
        ...
        rl_event_count = atomic_read(&(xdebug_struct.ratelimit_event_count));
        if (ret >= 6) {
            rl_event_count = opt4;
            xpc_debug(0, "Setting rate limit event count to %d\n", rl_event_count);
        }

        rl_timeout_us = atomic_read(&(xdebug_struct.ratelimit_timeout_us));
        if (ret >= 7) {
            rl_timeout_us = opt5;
            xpc_debug(0, "Setting rate limit timeout to %d us\n", rl_timeout_us);
        }

        if (xdebug_configure(ipc_dev_mask, thresh, period_us, rl_event_count,
            rl_timeout_us) == 0) {
                xdebug_start();
        }
    }
    ...
    for (i = 0; i < XPC_IPC_NUM_MAILBOXES; ++i) {
        if (dinfo->active_source[i] != 0) {
            xpc_debug(0,
                      ...
                      i, dinfo->active_source[i], dinfo->active_target[i],
                      dinfo->mode_reg[i], dinfo->mstatus_reg[i], dinfo->send_reg[i],
                      dinfo->data_reg_0_1_2_6[i][0], dinfo->data_reg_0_1_2_6[i][1],
                      dinfo->data_reg_0_1_2_6[i][2], dinfo->data_reg_0_1_2_6[i][3]);
        }
    }
...
static int xdebug_kthread(void *arg)
{
    ...
    while (!kthread_should_stop()) {
        // Sleep
        jiffies =
            usecs_to_jiffies(atomic_read(&(xdebug_struct.sample_period_us)));
        ...
        ipc_dev_mask = atomic_read(&(xdebug_struct.ipc_dev_mask));
        ratelimit_threshold = atomic_read(&(xdebug_struct.ratelimit_event_count));
        ratelimit_window_us = atomic_read(&(xdebug_struct.ratelimit_timeout_us));
        ...
        for (ipc = 0; ipc < XPC_NUM_IPC_DEVICES; ++ipc) {
            if ((ipc_dev_mask >> ipc) & 0x1) {
                ...
                if (rate_limited[ipc]) {
                    ...
                    now_ns = ktime_get_ns();
                    if ((now_ns - rl_window_start_ns[ipc]) > ratelimit_window_ns) {
                ...
                if (!rate_limited[ipc]) {
                    ...
                    if (rl_event_count[ipc] >= ratelimit_threshold) {
                        rate_limited[ipc] = true;
                        rl_window_start_ns[ipc] = ktime_get_ns();
                        xpc_debug(0,
                            "************ Rate limiting "
                            "IPC[%u] logs for %lld usecs ***********\n",
                            ipc, ratelimit_window_us);
                    }
                }
                ...
                if (!rate_limited[ipc]) {
                    ...
                    if (dinfo.num_in_use >
                        atomic_read(&(xdebug_struct.contention_thresh))) {
                        rl_event_count[ipc]++;
...
void __xdebug_event_record_all(uint8_t ipc_idx, uint8_t event_id, uint32_t value)
{
	uint64_t ts;
	uint8_t mb_idx;
