...
int xstat_kthread(void *arg)
{
    ...
    while (!kthread_should_stop()) {
        ...
        xpc_debug(3, "%s: Woke up, state: %u wait count: %u\n", __func__,
                  xstat_struct.state, xstat_struct.wait_count);
        ...
        if (xstat_struct.wait_count) {
            ...
            // Send request
            xstat_struct.req.ret_code = xstat_get_core_info_internal(
                xstat_struct.req.target_core, &(xstat_struct.req.core_info));
            spin_lock(&(xstat_struct.lock));
            xpc_debug(2, "%s: Got response from %u. ret:%u Core state:%u\n",
        ...
        if (xstat_struct.state_changed_info.state_changed) {
            ...
            xpc_debug(2, "%s: Sending state change message", __func__);
            spin_unlock(&(xstat_struct.lock));
            ret = xstat_send_state_change(xstat_struct.state_changed_info);
            spin_lock(&(xstat_struct.lock));
        ...
        xpc_debug(3, "%s: Signaling those who are waiting, wait count: %u\n",
                 ...
                 const char *buf, size_t count)
{
    ...
    if (user_state >= XSTAT_STATE_OFFLINE && user_state < XSTAT_STATE_INVALID) {
...
ssize_t wearable_state_show(struct class *class, struct class_attribute *attr,
                        char *buf)
{
    int ret = sprintf(buf, "%s\n", xstat_wearable_state_str(xstat_struct.cvip_core_info.wearable_state));
                 ...
                 const char *buf, size_t count)
{
    ...
    if (user_state >= XSTAT_WEARABLE_STATE_OFFLINE && user_state < XSTAT_WEARABLE_STATE_INVALID) {
...
int xstat_set_state(enum xstat_state state)
{
    ...
    xpc_debug(0, "%s: Changing xstat state from %s(%u) to %s(%u).\n", __func__,
              xstat_state_str(old_state), old_state,
...
int xstat_set_wearable_state(enum xstat_wearable_state state)
{
    ...
    xpc_debug(0, "%s: Changing wearable state from %s(%u) to %s(%u).\n", __func__,
              xstat_wearable_state_str(xstat_struct.cvip_core_info.wearable_state),
              xstat_struct.cvip_core_info.wearable_state, xstat_wearable_state_str(state), state);
                            ...
                            uint32_t timeout_ms)
{
    ...
    if (xstat_struct.state == THREAD_IDLE && xstat_struct.wait_count == 0) {
        ...
        xpc_debug(2, "%s: New request received, waking xstat kthread!\n",
                  __func__);
        spin_unlock(&(xstat_struct.lock));
        wake_up_process(xstat_struct.task);
    }
    ...
    start_ts = ktime_get();
    status = wait_event_interruptible_timeout(xstat_struct.wq,
                                              xstat_struct.state == THREAD_IDLE,
                                              timeout_ms * HZ / 1000);
    stop_ts = ktime_get();
                                 ...
                                 struct xstat_core_info *info)
{
    ...
    if (rsp_msg.target_core != source_core) {
        xpc_debug(
            0, "%s: Received message with invalid target. Expected:%u Got:%u\n",
            __func__, source_core, rsp_msg.target_core);
        return -EBADMSG;
    }
    if (rsp_msg.source_core != target_core) {
        xpc_debug(
            0, "%s: Received message with invalid source Expected:%u Got:%u\n",
    ...
    xpc_debug(1, "%s: Got state response from %u, core_state: %u\n", __func__,
              rsp_msg.source_core, xstat_rsp->core_info.core_state);
                               ...
                               union xpc_platform_channel_message *rsp_msg)
{
    ...
    if (xstat_cmd->key != PING_MAGIC_KEY) {
        ...
        if (XPC_CORE_ID == CORE_ID_A55 && xstat_cmd->state_changed_info.x86_state) {
            // On the cvip, so set the x86 state and notify the x86 sysfs
            xstat_struct.x86_core_info.core_state = xstat_cmd->core_info.core_state;
            xstat_sysfs_entry = XSTAT_X86_SYSFS;
            xpc_debug(1, "%s: x86 state changed to %s(%u).\n",
                __func__,
                xstat_state_str(xstat_cmd->core_info.core_state),
                xstat_cmd->core_info.core_state);
        }
        if (XPC_CORE_ID == CORE_ID_X86 && xstat_cmd->state_changed_info.cvip_state) {
            // On the x86, so set the cvip state and notify the cvip sysfs
            xstat_struct.cvip_core_info.core_state = xstat_cmd->core_info.core_state;
        ...
        if(xstat_cmd->state_changed_info.wearable_state) {
            ...
            xstat_struct.cvip_core_info.wearable_state = xstat_cmd->core_info.wearable_state;
            xstat_sysfs_entry = XSTAT_WEARABLE_SYSFS;
            xpc_debug(1, "%s: wearable state changed to %s(%u).\n",
                __func__,
                xstat_wearable_state_str(xstat_cmd->core_info.wearable_state),
...
int xstat_init(struct class *xpc_class)
{
    ...
    for (i = 0; i < XSTAT_SYSFS_ENTRY_COUNT; i++) {
        xstat_sysfs_kn[i] = sysfs_get_dirent(class_kn, xstat_sysfs_names[i]);
