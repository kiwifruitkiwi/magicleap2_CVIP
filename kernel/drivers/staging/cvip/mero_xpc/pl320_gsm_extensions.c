...
static uintptr_t ipc_ext_global_baseaddr[5] = {
    0x0,
    0x0,
    GSM_RESERVED_IPC_IPC2_GLOBAL_EXT_INFO_START,
    GSM_RESERVED_IPC_IPC3_GLOBAL_EXT_INFO_START,
    0x0
};
...
static uintptr_t ipc_ext_mailbox_baseaddr[5] = {
    0x0,
    0x0,
    GSM_RESERVED_IPC_IPC2_MAILBOX_EXT_INFO_START,
    GSM_RESERVED_IPC_IPC3_MAILBOX_EXT_INFO_START,
    0x0
};
...
Pl320HalStatus pl320_gsm_initialize(pl320_device_info *info)
{
...
#if !defined(__i386__) && !defined(__amd64__)
	{
		...
		gsm_raw_write_32(&(ext_p->mailbox_status), ~(info->mailbox_enabled));
		// ARM dictates whether all cores will use gsm extensions
		gsm_raw_write_32(&(ext_p->use_fast_extensions), PL320_USE_GSM_FAST_EXTENSIONS);
		gsm_raw_write_32(&(ext_p->use_safe_extensions), PL320_USE_GSM_SAFE_EXTENSIONS);
	...
	info->use_fast_ext =
		gsm_raw_read_32(&(ext_p->use_fast_extensions)) == 0 ?
			false :
			true;

	if (info->use_fast_ext) {
		// Signify that this core has initialized pl320 gsm fast extensions
		primitive_atomic_bitwise_set(&(ext_p->core_mask), 0x1 << (FAST_CORE_MASK_START + CURRENT_CORE_ID));
	}

	// Determine if the master (ARM) has designated the use of GSM safe extensions
	info->use_safe_ext =
		gsm_raw_read_32(&(ext_p->use_safe_extensions)) == 0 ?
			false :
			true;

	if (info->use_safe_ext) {
		// Signify that this core has initialized pl320 gsm safe extensions
		primitive_atomic_bitwise_set(&(ext_p->core_mask), 0x1 << (SAFE_CORE_MASK_START + CURRENT_CORE_ID));
	}
...
{
	struct pl320_gsm_ipc_global_info_s *ext_p = (struct pl320_gsm_ipc_global_info_s*)info->ipc_ext_global_base;
