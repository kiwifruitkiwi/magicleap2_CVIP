...
int xpc_send_queue_internal(struct XpcQueueInfo *message, enum xpc_client_api_mode client_mode)
{
	...
	for (i = 0; i < XPC_NUM_IPC_DEVICES; i++) {
		...
		mailboxes[mailbox_id].mailbox_info.pl320_info =
			...
			xpc_set_global_mailbox_pid(mailbox_id, current->pid,
						   XPC_REQUEST_IS_SOURCE, client_mode);
                                      ...
                                      int *claimed_mailbox)
{
    ...
    status =
        xpc_queue_check_subchannel_match(vchannel, schannel, claimed_mailbox);
    if (status == 0) {
                                      ...
                                      uint8_t *process_subchannels)
{
    ...
    if (vchannel < XPC_NUM_VCHANNELS) {
        ...
        check = (process_subchannels[CHANNEL_MASK_OFFSET(vchannel, schannel)] &
                            ...
                            uint8_t *process_subchannels)
{
    ...
    u_registered =
        registered_user_subchannels[vchannel][SUB_CHANNEL_BYTE(schannel)] &
        SUB_CHANNEL_MASK(schannel);
                              ...
                              uint8_t *process_subchannels)
{
    ...
    if (p_registered) {
        ...
        if (!k_registered) {
            while (xpc_queue_check_subchannel_match(vchannel, schannel,
                                                    &mailbox) != 0) {
                mailboxes[mailbox].mailbox_info.target_state = MAILBOX_STATE_FREE;
...
int xpc_queue_register(XpcChannel vchannel, XpcSubChannel schannel)
{
    ...
    k_registered =
        registered_kernel_subchannels[vchannel][SUB_CHANNEL_BYTE(schannel)] &
        SUB_CHANNEL_MASK(schannel);
