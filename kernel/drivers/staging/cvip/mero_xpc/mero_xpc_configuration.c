...
#define TGT_CORE_IS_VALID(core_id)	\
	(IPC_VALID_TARGET_MASK & (0x1 << (core_id)))
...
uint32_t ipc_channel_id_masks[XPC_NUM_IPC_DEVICES][XPC_NUM_CORES] = {
	...
	{ CLUSTER_1_IPC3_CHANNELS, CLUSTER_1_IPC3_CHANNELS << 2,
	  ...
	  CLUSTER_0_IPC3_CHANNELS << 16, CLUSTER_0_IPC3_CHANNELS << 18, 0x3 << 21,
	  0x3 << 14 },
	...
	{ CLUSTER_1_IPC4_CHANNELS, CLUSTER_1_IPC4_CHANNELS << 2,
	  ...
	  CLUSTER_0_IPC4_CHANNELS << 16, CLUSTER_0_IPC4_CHANNELS << 18, 0x3 << 21,
	  0x3 << 14 }
};
...
int xpc_compute_route(uint32_t destinations, struct xpc_route_solution *route)
{
	...
	for (tgt_core = IPC_VALID_TARGET_MIN, destinations >>= IPC_VALID_TARGET_MIN;
