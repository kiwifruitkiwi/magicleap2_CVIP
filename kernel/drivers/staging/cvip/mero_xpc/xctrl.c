...
static long xctrl_fops_ioctl(struct file *file, unsigned int cmd, unsigned long data) {
    xpc_debug(1, "%s\n", __func__);
    return 0;
}

static ssize_t xctrl_fops_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos) {
    int ret;
    int status;
    XpcChannel channel;
    loff_t pos;
    ...
    status = xpc_notification_recv(channel, rsp, XPC_MAX_NOTIFICATION_LENGTH);
    if (status != 0) {
...
static ssize_t xctrl_fops_write(struct file *filp, const char __user *buf, size_t count, loff_t *pos) {
    int ret;
    int status;
    XpcChannel channel;
    char cmd;
    ...
    ret = sscanf(str_buf, "%c:%i:%" xstr(XCTRL_BUFF_SIZE) "s", &cmd, &target_mask, xpc_msg);

    // 2 or 3 to allow sending a notification with no data
    if (ret != 2 && ret != 3) {
        xpc_debug(0, "%s: invalid input:%s. Expected format x:target_mask[:data]\n",
            __func__, str_buf);
        return -EINVAL;
    }
    ...
    if (((BIT(CORE_ID_A55) & target_mask) == 0) &&
        ...
        xpc_debug(0, "%s: invalid target mask:0x%x. Only 0x%lx(ARM) or 0x%lx(x86) allowed\n",
            __func__, target_mask, BIT(CORE_ID_A55), BIT(CORE_ID_X86));
        return -EINVAL;
    }
    ...
    if (msg_length > XPC_MAX_NOTIFICATION_LENGTH) {
        xpc_debug(0, "%s: invalid message length:%zu. Max allowed:%u Message:%s\n",
            __func__, msg_length, XPC_MAX_NOTIFICATION_LENGTH, xpc_msg);
        return -EINVAL;
    }

    xpc_debug(2, "%s: sending %s len: %zu\n", __func__, xpc_msg, msg_length);

    // Send notification
    status = xpc_notification_send(channel, target_mask, xpc_msg,
        XPC_MAX_NOTIFICATION_LENGTH, XPC_NOTIFICATION_MODE_POSTED);
    if (status != 0) {
...
static unsigned int xctrl_fops_poll(struct file * filp, struct poll_table_struct *wait) {
    XpcChannel channel;
    unsigned int mask = 0;
...
int xctrl_create_fs_device(struct class *xpc_class) {
    ...
    ret = alloc_chrdev_region(&xctrl_first_dev, 0, XCTRL_DEV_COUNT, XCTRL_DEV_NAME);
    if (ret < 0) {
    ...
    for (i = 0; i < XCTRL_DEV_COUNT; i++) {
        if (notif_dev_list[i].enable) {
            ...
            ret = cdev_add(&notif_dev_list[i].cdev, notif_dev_list[i].dev, 1);
            if (ret < 0) {
                xpc_debug(0, "xctrl cdev_add error: %d minor num:%d\n", ret, i);
                return ret;
            }

            if (notif_dev_list[i].dev_name[0] != 0) {
                // Custom name
                snprintf(name_buff, XCTRL_BUFF_SIZE, "%s_notif_%s",
                    XCTRL_DEV_NAME, notif_dev_list[i].dev_name);
            } else {
                // Default name
                snprintf(name_buff, XCTRL_BUFF_SIZE, "%s_notif_%d",
                    XCTRL_DEV_NAME, i);
            }

            // Create device
            notif_dev_list[i].device = device_create(xpc_class, NULL,
                notif_dev_list[i].dev, NULL, "%s", name_buff);
            if (IS_ERR(&notif_dev_list[i].device)) {
                xpc_debug(0, "xctrl device_create minor num:%d\n", i);
                return -1;
            }
