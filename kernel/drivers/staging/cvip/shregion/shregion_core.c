...
// TODO(sadadi): Fake cvcorename kernel support for now
#define CVCORE_NAME_DECL(x) CVCORE_NAME_ ## x
...
static int ion_buffer_get_phys_addr(struct shr_meta_priv *meta_node, int offset);
...
static int shregion_slc_sync(struct shr_slc_sync_meta *slc_meta) {
	if (slc_meta->flags & SHREGION_SLC_SYNC_CLEAN) {
		return syscache_clean_range(slc_meta->start_addr,
			slc_meta->start_addr + slc_meta->size_bytes);

	} else if (slc_meta->flags & SHREGION_SLC_SYNC_FLUSH) {
		return syscache_flush_range(slc_meta->start_addr,
			slc_meta->start_addr + slc_meta->size_bytes);

	} else if (slc_meta->flags & SHREGION_SLC_SYNC_INVAL) {
		return syscache_inv_range(slc_meta->start_addr,
			slc_meta->start_addr + slc_meta->size_bytes);
	}
...
static int shregion_dmac_register_user_buf(struct dmac_reg_user_buf_meta *dmac_user_buf_meta) {
	...
	if (dmac_user_buf_meta->fixed_dmac_iova != 0) {
		...
		rsvd_iova = reserve_iova(dmac_iova_domain, rsv_req_pfn_lo, rsv_req_pfn_hi);

		if (!rsvd_iova) {
			__free_iova(dmac_iova_domain, rsvd_iova);
		...
		dmac_ioaddr = alloc_iova_fast(dmac_iova_domain, npages, PHYS_PFN(DMA_BIT_MASK(32)), true);
		dmac_ioaddr = dmac_ioaddr << PAGE_SHIFT;
	}
...
static int shregion_register_dmac(struct shregion_metadata_drv* meta_drv)
{
	...
	if (meta_drv->meta.type == SHREGION_C5_NCM_0 || meta_drv->meta.type == SHREGION_C5_NCM_1) {
		...
		if (!ncm_iova_map_alloc) {
			...
			ncm_iova_base = alloc_iova_fast(dmac_iova_domain, shr_npages, PHYS_PFN(DMA_BIT_MASK(32)), true);
			ncm_iova_base <<= PAGE_SHIFT;
		...
		if (meta_drv->meta.flags & SHREGION_FIXED_DMAC_IO_VADDR) {
			...
			dmac_ioaddr = alloc_iova_fast(dmac_iova_domain, shr_npages, PHYS_PFN(DMA_BIT_MASK(32)), true);
			dmac_ioaddr = dmac_ioaddr << PAGE_SHIFT;
		}
	}
...
__maybe_unused static int test_cdp_alloc(void) {
	...
	meta.name = CVCORE_NAME_DECL(CDP_DCPRS_KERNEL_TEST);
