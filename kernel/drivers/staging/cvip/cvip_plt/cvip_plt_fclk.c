...
extern void dsp_helper_off_register_notify(struct notifier_block *nb);
extern void dsp_helper_off_deregister_notify(struct notifier_block *nb);
...
static int cvip_fclk_on_dsp_core_shutdown(struct notifier_block *nb, unsigned long core_id, void *data)
{
	int status = 0;
	struct fclk_req req;
...
int cvip_fclk_init(struct kobject *kobj)
{
	...
	dsp_helper_off_register_notify(&dsp_core_shutdown_nb);
...
void cvip_fclk_exit(void)
{
	...
	dsp_helper_off_deregister_notify(&dsp_core_shutdown_nb);
}
