...
#ifdef CONFIG_ARM64_GDB_ACCESS
int custom_generic_access_phys(struct vm_area_struct *vma, unsigned long addr,
...
static const struct vm_operations_struct custom_vm_ops = {
	.access = custom_generic_access_phys,
};
#endif
...
/* checking whether this ion buffer needs device type access */
bool ion_buffer_device(struct ion_buffer *buffer)
{
	return !!(buffer->flags & ION_FLAG_CVIP_DEVICE_TYPE);
}

/* checking whether this ion buffer is GSM */
static bool ion_buffer_gsm(struct ion_buffer *buffer)
{
	return !!(buffer->heap->id == ION_HEAP_CVIP_SRAM_GSM_ID);
}
...
static int ion_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	/*
	 * Mark device access for buffer setup with DEVICE type or if it's GSM,
	 * or if the buffer is not ARM cached, then we should always mark
	 * the access as non-cached with write combine enabled.
	 */
	if (ion_buffer_device(buffer) || ion_buffer_gsm(buffer))
...
static int ion_dma_buf_begin_cpu_access_no_lock(struct dma_buf *dmabuf,
					     enum dma_data_direction direction)
{
	struct ion_buffer *buffer = dmabuf->priv;
	struct ion_dma_buf_attachment *a;
...
static int ion_dma_buf_end_cpu_access_no_lock(struct dma_buf *dmabuf,
					     enum dma_data_direction direction)
{
	struct ion_buffer *buffer = dmabuf->priv;
	struct ion_dma_buf_attachment *a;
...
static const struct dma_buf_ops dma_buf_ops = {
	...
	.begin_cpu_access_no_lock = ion_dma_buf_begin_cpu_access_no_lock,
	.end_cpu_access_no_lock = ion_dma_buf_end_cpu_access_no_lock,
