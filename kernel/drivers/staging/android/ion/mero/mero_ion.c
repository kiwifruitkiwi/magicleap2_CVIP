...
#define MAX_NR_OF_XPORT_HEAPS (3)

static int num_heaps;
static struct ion_heap **mero_heaps;
static bool mlnet_heap_xport_created;
static bool shared_heap_xport_created;
static bool dump_heap_xport_created;
...
static struct ion_heap_desc ion_heap_meta[] = {
	...
	{
		.id	= ION_HEAP_CVIP_DDR_PRIVATE_CONTIG_ID,
		.name = ION_SYSTEM_CONTIG_HEAP_NAME,
	},
	{
		.id	= ION_HEAP_CVIP_SRAM_GSM_ID,
		.name = ION_DSP_HEAP_NAME,
	},
};
...
int mero_ion_heap_xport_destroy(void)
{
	...
	if (!shared_heap_xport_created) {
		pr_err("%s: x-port heap not existed\n", __func__);
		return -1;
	}

	for (i = num_heaps; num_heaps + MAX_NR_OF_XPORT_HEAPS; i++) {
		mero_ion_heap_destroy(mero_heaps[i]);
		pr_info("ION heap %d xport has been removed\n", i);
	}
	mlnet_heap_xport_created = false;
	shared_heap_xport_created = false;
	dump_heap_xport_created = false;

	return 0;
}
EXPORT_SYMBOL(mero_ion_heap_xport_destroy);

static int check_xport_window(phys_addr_t paddr, size_t size)
{
	phys_addr_t end_paddr = paddr + size - 1;
	int ret = 0;

	/* check address and size is within a valid x-port window */
	if (paddr < MERO_CVIP_XPORT_HEAP_ID1_BASE ||
	    paddr >= (MERO_CVIP_XPORT_HEAP_ID1_BASE
		      + MERO_CVIP_XPORT_HEAP_ID1_SIZE)) {
		pr_err("error paddr %pa outside xport window\n", &paddr);
		ret = -1;
	} else if ((paddr + size) > (MERO_CVIP_XPORT_HEAP_ID1_BASE
				+ MERO_CVIP_XPORT_HEAP_ID1_SIZE)) {
		pr_err("error paddr %pa outside xport window\n", &end_paddr);
		ret = -1;
	}
 ...
 */
int mero_ion_heap_xport_create(phys_addr_t paddr, size_t size, uint32_t type)
{
	...
	if (type == mlnet) {
		if (mlnet_heap_xport_created) {
			pr_err("%s: mlnet error x-port heap already created\n",
			       __func__);
			return -1;
		}
	} else if (type == shared) {
		if (shared_heap_xport_created) {
			pr_err("%s: shared error x-port heap already created\n",
			       __func__);
			return -1;
		}
	} else if (type == dump) {
		if (dump_heap_xport_created) {
			pr_err("%s: shared error x-port heap already created\n",
			       __func__);
			return -1;
		}
	} else {
		pr_err("%s: error unknown buffer type for x-port heap\n",
		       __func__);
		return -1;
	}
	...
	ret = check_xport_window(paddr, size);
	if (ret == -1)
		return ret;
	...
	switch (type) {
	case mlnet:
		heap_data.id = ION_HEAP_X86_CVIP_MLNET_ID;
		heap_data.name = ION_MLNET_HEAP_NAME;
		heap = num_heaps + 1;
		break;
	case shared:
	...
	case dump:
		heap_data.name = ION_XPORT_HEAP_NAME;
		heap_data.id   = ION_HEAP_CVIP_DUMP_ID;
		heap = num_heaps + 2;
		break;
	default:
		break;
	...
	mero_heaps[heap] = mero_ion_heap_create(&heap_data);

	if (IS_ERR_OR_NULL(mero_heaps[heap])) {
		mero_heaps[heap] = NULL;
		goto err_out;
	}

	pr_info("ION heap %s created at %pa with size %zx\n",
		heap_data.name, &heap_data.base, heap_data.size);
	ion_device_add_heap(mero_heaps[heap]);

	if (type == mlnet)
		mlnet_heap_xport_created = true;
	else if (type == shared)
		shared_heap_xport_created = true;
	else if (type == dump)
		dump_heap_xport_created = true;
	return 0;
...
static int mero_ion_probe(struct platform_device *pdev)
{
	...
	/* have extra three for x-port heaps that may register later
	 * at run-time
	 */
	mero_heaps = kcalloc(pdata->nr + MAX_NR_OF_XPORT_HEAPS,
...
static int mero_ion_remove(struct platform_device *pdev)
{
	...
	if (mlnet_heap_xport_created) {
		mero_ion_heap_destroy(mero_heaps[num_heaps + 1]);
		mlnet_heap_xport_created = false;
	}

	if (dump_heap_xport_created) {
		mero_ion_heap_destroy(mero_heaps[num_heaps + 2]);
		dump_heap_xport_created = false;
	}
