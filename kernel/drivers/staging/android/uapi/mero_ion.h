...
enum ion_heap_ids {
	...
	ION_HEAP_X86_SYSTEM_ID  = 6,
	ION_HEAP_X86_CVIP_MLNET_ID  = 7,
	...
	ION_HEAP_CVIP_SRAM_GSM_ID = 29,
	ION_HEAP_CVIP_DDR_PRIVATE_CONTIG_ID = 30,
	ION_HEAP_CVIP_DUMP_ID  = 31,
	MAX_ION_HEAP_NUM = 32,
};
...
#define ION_MLNET_HEAP_NAME     "mlnet"
...
#define ION_FMR2_10_HEAP_NAME	"fmr2_10"

#define ION_SYSTEM_CONTIG_HEAP_NAME	"system_contig"

/*
 * The XPORT aperture is a 32GB address space that the cvip can
 * use to map in memory from the x86, starting at 0x4000000000.
 * We can't map in the entire 32GB address range, so we will map in
 * smaller heaps and use the XPORT_BASE and SIZE to enure that those heaps
 * are within the xport address range.
 */
#define MERO_CVIP_ADDR_XPORT_BASE		0x4000000000
#define MERO_CVIP_XPORT_APERTURE_SIZE		0x800000000

/* The first xport heap is used for shared ddr between the CVIP and the x86
 * this includes, mlnet, pads and the crash kernel. This is a 64MB area, for now
 * we map in 256 MB incase this heap needs to be expanded.
 *
 * Eventually read the base and size from the dts for each heap we creat inside
 * the xport aperature
 */
#define MERO_CVIP_XPORT_HEAP_ID1_BASE	0x40f0000000
#define MERO_CVIP_XPORT_HEAP_ID1_SIZE	0x100000000
...
/*
 * flag to indicate if this buffer should have device type access
 */
#define ION_FLAG_CVIP_DEVICE_TYPE		ION_BIT(24)
...
extern void notify_xport_ready(__u32 type);
