					...
					struct drm_dp_vcpi *vcpi)
{
	...
	vcpi_ret = find_first_zero_bit(&mgr->vcpi_mask, min_t(uint32_t,
			mgr->max_payloads + 1, BITS_PER_LONG));
...
int drm_dp_mst_atomic_check(struct drm_atomic_state *state)
{
	...
	for_each_new_mst_mgr_in_state(state, mgr, mst_state, i) {
		// skip checking for destroyed MST port
		if (!mgr->mst_state)
			continue;
