...
static void _stop(struct pl330_thread *thrd)
{
	...
	/* update time counter on stop */
	thrd->tcount = local_clock() - thrd->tcount;
...
static int pl330_update(struct pl330_dmac *pl330)
{
	...
	for (ev = 0; ev < pl330->pcfg.num_events; ev++) {
		if (val & (1 << ev)) { /* Event occurred */
			...
			/* update time counter for perf measurement */
			thrd->tcount = local_clock() - thrd->tcount;
...
#define NAME_DMA0  "d0000000.pdma"
#define NAME_DMA1  "d0800000.pdma"
static struct device *dma_dev0;
void helper_dma_iommu_share(struct pl330_dmac *pl330)
{
	struct device *dma_dev1 = NULL;

	if (!strcmp(dev_name(pl330->ddma.dev), NAME_DMA0)) {
		/* keep DMA0 device pointer to dma_dev0 */
		dma_dev0 = pl330->ddma.dev;
		return;
	}

	if (!strcmp(dev_name(pl330->ddma.dev), NAME_DMA1)) {
		/* get DMA1 device pointer */
		dma_dev1 = pl330->ddma.dev;

		/* apply DMA0 dma_ops/iommu setup to DMA1 */
		if (!dma_dev1->dma_ops)
			dma_dev1->dma_ops = dma_dev0->dma_ops;
		if (!dma_dev1->iommu_group)
			dma_dev1->iommu_group = dma_dev0->iommu_group;
		if (!dma_dev1->iommu_fwspec)
			dma_dev1->iommu_fwspec = dma_dev0->iommu_fwspec;
		if (!dma_dev1->iommu_param)
			dma_dev1->iommu_param = dma_dev0->iommu_param;
	}
}

int share_dma_device(struct device *sh_dev)
{
	if (!dma_dev0)
		return -ENOMEM;

	sh_dev->dma_ops = dma_dev0->dma_ops;
	sh_dev->iommu_group = dma_dev0->iommu_group;
	sh_dev->iommu_fwspec = dma_dev0->iommu_fwspec;
	sh_dev->iommu_param = dma_dev0->iommu_param;

	return 0;
}

int pl330_share_dma_device(struct device *sh_dev)
{
        return share_dma_device(sh_dev);
}
EXPORT_SYMBOL(pl330_share_dma_device);
#endif
...
static int dmac_alloc_resources(struct pl330_dmac *pl330)
{
...
#ifdef CONFIG_ARCH_MERO_CVIP
	/* helper function to share DMA0 iommu for DMA1 */
	helper_dma_iommu_share(pl330);
#endif
	 ...
	 */
	if (pl330->pcfg.privileged)
		attrib = DMA_ATTR_PRIVILEGED;
...
static dma_cookie_t pl330_tx_submit(struct dma_async_tx_descriptor *tx)
{
	...
	while (!list_empty(&last->node)) {
		...
		if (pch->cyclic) {
			...
			desc->txd.callback_result = last->txd.callback_result;
...
pl330_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	for (i = 0; i < AMBA_NR_IRQS; i++) {
		...
		if (irq) {
			...
			if (ret)
				return ret;
			irq_set_affinity(irq, cpumask_of(irq % nr_cpu_ids));
		} else {
			break;
		}
	}
	...
	pcfg = &pl330->pcfg;
	if (device_property_read_u32(&adev->dev, "privileged",
				     &pcfg->privileged))
		pcfg->privileged = 1;
	...
	dev_info(&adev->dev,
		 "\tperi_ns-0x%x mode_ns=0x%x irq_ns-0x%x\n", pcfg->peri_ns,
		 pcfg->mode & DMAC_MODE_NS, pcfg->irq_ns);
