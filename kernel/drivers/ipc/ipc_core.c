...
#include "ipc.h"
#include "linux/cvip/cvip.h"
#include "linux/mero/cvip/cvip_nova_common.h"
#ifdef CONFIG_CVIP_PLT
...
int send_cvip_panic_message(void)
{
	...
	pm_message.length = 0;
	pm_message.message_id = CVIP_PANIC;

	return ipc_sendmsg(g_ipc, &pm_message);
}
...
{
	if (allow_reboot)
		kernel_restart_remote(NULL);
	else
		pr_info("Remote reboot disabled\n");
}

void handle_nova_panic(struct mbox_message *msg)
{
	pr_info("nova panic - reboot\n");
	kernel_restart(NULL);
}

void handle_nova_alive(struct mbox_message *msg)
{
	pr_info("nova alive\n");
	notify_nova_online();
}
...
int send_cvip_s2idle_allow_message(bool is_allowed)
{
	struct ipc_message pm_message;
	...
	pm_message.payload[0] = CVIP_QUERY_S2IDLE_ACK;

	if (is_allowed)
		pm_message.payload[1] = CVIP_ALLOW_S2IDLE;
	else
		pm_message.payload[1] = CVIP_BLOCK_S2IDLE;
...
static int handle_low_power(u32 mode)
{
	...
	case X86_S0:
	{
		...
		/* Notify listening drivers that we are resuming */
		cvip_notify_resume();

		/* Handle scenarios where x86 goes into and out of S2Idle multiple times.
		 * This could result in multiple entries into this function while CVIP
		 * userspace has only reported it's state update once to sysfs.
		 */
		if (get_cvip_pm_state() != PM_STATE_NORMAL_OPERATION) {
			ret = cvip_pm_wait_for_completion();
			if (ret < 0) {
				pr_err("CVIP failed to transition to Normal in time\n");
				break;
			}

			cvip_pm_reinit_completion();
			pm_state = get_cvip_pm_state();
			if (pm_state != PM_STATE_NORMAL_OPERATION) {
				pr_err("Mismatch b/t CVIP and x86 reported PM states (st=%d)\n",
					pm_state);
				ret = -EPERM;
				break;
			}
		...
		pr_info(" %s: CVIP sending Normal ack to x86\n", __func__);
		if (ipc_sendmsg(g_ipc, &pm_message) < 0)
			pr_err(" %s: failed to send Normal ack to x86\n", __func__);

		break;
	}
	case X86_IDLE:
	{
		pr_info(" %s: X86 is entering standby idle...\n", __func__);
		cvip_notify_s2idle();
		 ...
		 * userspace has only reported it's state update once to sysfs.
		 */
		if (get_cvip_pm_state() != PM_STATE_S2IDLE_SLEEP) {
			ret = cvip_pm_wait_for_completion();
			if (ret < 0) {
				pr_err("CVIP failed to transition to S2Idle in time\n");
				break;
			}
		...
		pr_info(" %s: CVIP sending S2Idle ack to x86\n", __func__);
		if (ipc_sendmsg(g_ipc, &pm_message) < 0)
			pr_err(" %s: failed to send S2Idle ack to x86\n", __func__);
...
static int handle_cvip_query_s2idle(void)
{
	int ret, state, allow;

	state = get_cvip_pm_state();

	switch (state) {
	case PM_STATE_NORMAL_OPERATION:
		pr_info("Request to transition from Normal to S2Idle\n");
		allow = 1;
		break;
	case PM_STATE_STANDBY_DOZE:
		pr_info("Request to transition from Doze to S2Idle\n");
		allow = 1;
		break;
	case PM_STATE_S2IDLE_SLEEP:
		pr_info("Request to transition from S2Idle to S2Idle\n");
		allow = 1;
		break;
	default:
		pr_info("Request to transition from %d to S2Idle\n", state);
		allow = 0;
		break;
	}

	if (allow) {
		pr_info("Sending allow S2Idle msg\n");
		ret = send_cvip_s2idle_allow_message(true);
		if (ret < 0)
			pr_err("Failed to send s2idle allow msg\n");
	} else {
		pr_info("Sending not allowed S2Idle msg\n");
		ret = send_cvip_s2idle_allow_message(false);
		if (ret < 0)
			pr_err("Failed to send s2idle allow msg\n");
	}
...
int handle_pm_event(struct mbox_message *msg)
{
	...
	pr_info("Handling PM IPC event (0x%X)\n", event);
	...
	case CVIP_QUERY_S2IDLE:
		pr_debug("Query s2idle message received\n");
		ret = handle_cvip_query_s2idle();
		break;
#endif
	default:
		ret = -EINVAL;
...
#if defined(CONFIG_IPC_XPORT) && defined(CONFIG_CRASH_DUMP)
...
int handle_install_dump_kernel(struct mbox_message *msg)
{
	...
	alloc_fd = ion_alloc(dump_size, 1 << ION_HEAP_CVIP_DUMP_ID,
			     ION_FLAG_CACHED);
	if (alloc_fd < 0) {
...
#ifdef CONFIG_CVIP_PLT
	/* save this in cvip_plt */
	set_dump_buf_base(paddr);
#endif
	dma_buf_put(dma_buf);
...
int handle_message(struct mbox_message *msg)
{
	...
	case REBOOT_REQ:
		...
		break;
	case NOVA_PANIC:
		handle_nova_panic(msg);
		break;
	case NOVA_ALIVE:
		handle_nova_alive(msg);
		break;
#endif
	case SET_GPUVA:
...
void handle_xport_req(void *message)
{
	...
	case REGISTER_BUFFER:
	{
		...
		buffer_type = msg->msg_body[3];

		pr_debug("handle xport req %d\n", buffer_type);
		switch (buffer_type) {
		case mlnet:
			pr_debug("%s: mlnet xport_heap_paddr %pa, size %zx\n",
				 __func__, &xport_heap_paddr, xport_heap_size);
			mero_ion_heap_xport_create(xport_heap_paddr,
						   xport_heap_size,
						   mlnet);
#ifdef CONFIG_CVIP_PLT
			set_mlnet_base(xport_heap_paddr);
#endif
			break;
		case shared:
			pr_debug("%s: x86 shared xport_heap_paddr %pa, size %zx\n",
				 __func__, &xport_heap_paddr, xport_heap_size);
			mero_ion_heap_xport_create(xport_heap_paddr,
						   xport_heap_size,
						   shared);
#ifdef CONFIG_CVIP_PLT
			set_nova_shared_base(xport_heap_paddr);
#endif
			break;
		case dump:
			pr_debug("%s:dump heap xport_heap_paddr %pa, size %zx\n",
				 __func__, &xport_heap_paddr, xport_heap_size);
			mero_ion_heap_xport_create(xport_heap_paddr,
						   xport_heap_size,
						   dump);
		...
		default:
			err = 1;
			pr_err("unknown buffer type\n");
			break;
		}

#ifdef CONFIG_CVIP_PLT
		if (!err)
			notify_xport_ready(buffer_type);
#endif
		break;
	}
...
int rx_worker_thread_func(void *data)
{
	...
	pr_info("ipc rx worker thread started\n");
	...
	pr_info("ipc rx worker thread exit\n");
...
#ifdef CONFIG_IPC_VCAM_ISP
int get_prepost_mem(struct isp_ipc_device *dev, int cam_id)
#else
...
#ifdef CONFIG_IPC_VCAM_ISP
int get_stats_mem(struct isp_ipc_device *dev, int cam_id)
#else
