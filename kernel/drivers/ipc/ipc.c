...
int ipc_sendmsg(struct ipc_context *ipc, struct ipc_message *msg)
{
	...
	if (msg->message_id == CVIP_PANIC) {
		/* minimum to send a message to nova */
		mbox_send_message(chan->chan, (void *)mbox_msg);
		goto out;
	}
	...
	if (chan->sync) {
		...
		if (ret >= 0) {
			if (!completion_done(&msg_list->done)) {
				err = wait_for_completion_interruptible_timeout(
							 &msg_list->done,
							 MAX_SYNC_TIMEOUT);
				if (err <= 0) {
					pr_err("completion not successful\n");
					ret = err ? -ERESTARTSYS : -ETIMEDOUT;
				}
			}
		}
		put_mbox_message(chan, mbox_msg);
...
static int alive_thread_func(void *data)
{
	...
	cvip_plt_get_reboot_info(&boot_reason, &watchdog_count);

	while (!alive_sent) {
		pr_debug("cvip to x86 alive msg\n");
