...
static unsigned long scpi_clk_get_val(u16 clk_id)
{
	...
	if (ret) {
		pr_err("Failed (%d) to get clk for CLK id(%d)", ret, clk_id);
		return ret;
	}
	pr_info("Get clk for CLK id(%d) ====> %d MHz\n", clk_id, rate);
...
static int scpi_clk_set_val(u16 clk_id, unsigned long rate)
{
	...
	if (ret) {
		pr_err("Failed (%d) to set clk (%d MHz) for CLK id(%d)", ret,
				 cpu_to_le32(rate)/SCPI_1MHZ, clk_id);
	} else {
		pr_info("Set clk for CLK id(%d) ====> %d MHz\n", clk_id,
				 cpu_to_le32(rate)/SCPI_1MHZ);
	}
	return ret;
}
...
struct scpi_drvinfo *get_scpi_drvinfo(void)
{
	return scpi_info;
}
EXPORT_SYMBOL_GPL(get_scpi_drvinfo);
...
static int scpi_probe(struct platform_device *pdev)
{
	...
	dev_info(dev, "res:0x%llx, size:0x%llx\n", res->start, resource_size(res));
